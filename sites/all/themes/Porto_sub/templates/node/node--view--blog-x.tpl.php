...
<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix post post-large-image blog-single-post"<?php print $attributes; ?>>
  ...
  <?php endif; ?>
	  ...
	  <?php if ($display_submitted && !$teaser): ?>
	    ...
	    <div class="post-meta">
				<span class="post-meta-user"><i class="icon-user"></i> By 
				
                <?php
				$author_team_nid = db_query(
					"SELECT field_accout_team_member_ref_target_id  
					FROM {field_data_field_accout_team_member_ref}
				  	where entity_id = :uid", array('uid' => $uid ))->fetchField();
				if ($author_team_nid > 0) {
					$author_team_alias = drupal_get_path_alias('node/' . $author_team_nid);
					print  t("<a href='" . $author_team_alias . "'>" . $name . "</a>") ;
				} else {
					print $name;
				}
				?>
